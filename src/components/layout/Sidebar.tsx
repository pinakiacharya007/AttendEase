  ...
  return(
    <>
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
        {isMobile && (
          <button className="mobile-close-btn" onClick={()=>setSidebarOpen(false)} aria-label="Close navigation">✕</button>
        )}
      <div style={{padding:'18px 13px 14px',borderBottom:'1.5px solid var(--border)'}}>
            ...
            {g.items.map(item=>{
              ...
              return(
                <Link key={item.href} href={item.href} style={{textDecoration:'none',display:'block',marginBottom:'1px'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'7px',padding:'7px 9px',borderRadius:'9px',background:active?'linear-gradient(135deg,#ede9ff,#e4e0ff)':'transparent',color:active?'#3730a3':'var(--text-3)',fontWeight:active?'600':'500',fontSize:'12.5px',transition:'background .12s',cursor:'pointer'}}>
                    <span style={{fontSize:'13px',width:'17px',textAlign:'center',flexShrink:0}}>{item.icon}</span>
      ...
      <div style={{padding:'7px 7px 13px',borderTop:'1.5px solid var(--border)'}}>
        <div style={{display:'flex',alignItems:'center',gap:'7px',padding:'7px 9px',borderRadius:'9px',background:'var(--surface-2)',border:'1px solid var(--border)',marginBottom:'5px'}}>
          <div style={{width:'26px',height:'26px',borderRadius:'7px',background:'linear-gradient(135deg,#635bff,#a8a4ff)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:'700',color:'#fff',flexShrink:0}}>{s?.user?.name?.charAt(0).toUpperCase()}</div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:'11.5px',fontWeight:'600',color:'var(--text-1)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{s?.user?.name}</div>
        ...
        <button onClick={()=>signOut({callbackUrl:'/login'})} style={{width:'100%',padding:'6px',borderRadius:'8px',border:'1.5px solid var(--border)',background:'transparent',color:'var(--text-3)',fontSize:'12px',fontWeight:'600',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'5px',fontFamily:'inherit'}}>
          ↩ Sign Out
        </button>
      </div>
    </aside>
